...
class PageFrame(CTkFrame):
    ...
    def view(self):
        ...
        CTkLabel(self, text="Section patients", font=font(45), fg_color=Colors.Liver, corner_radius=20, width=430, height=75, text_color=Colors.White).pack(pady=15)

        patient_info = CTkFrame(
            self, width=360, height=560, fg_color=Colors.Cadet, corner_radius=20
        )
        patient_info.place(x=23, y=106)


        info= lambda text,y: CTkLabel(patient_info, font=font(20), text=text, text_color=Colors.White).place(x=20, y=y)
        info(f"Nom: Mr. ",30)
        ...
        CTkButton(self, text="Historique des visites", font=font(26), fg_color=Colors.Cadet, width=380, height=72, corner_radius=20, hover_color="#10162b", text_color=Colors.Mandarin).place(x=433,y=494)
        CTkButton(self, text="Ajouter une visite", font=font(26), fg_color=Colors.Mandarin, width=380, height=72, corner_radius=20, hover_color=Colors.Sepia, text_color=Colors.White).place(x=433,y=585)
        ...
        CTkButton(courbeDeCroissance, text="Courbes de croissance", fg_color=Colors.Cadet, text_color=Colors.White, height=55, width=375, corner_radius=10, font=font(28), hover_color=Colors.Mandarin, bg_color=Colors.Coral).place(x=0, y=0)
        CTkFrame(courbeDeCroissance, height=10, fg_color=Colors.Cadet, corner_radius=0).place(x=0,y=45, relwidth=1)
        imag = CTkImage(light_image=Image.open("assets/imgs/courbe de croissance.png"), size=(380, 305))
        CTkLabel(courbeDeCroissance, image=imag).place(x=0, y=43)
...
class PatientsList(CTkFrame):
    ...
    def load(self):
        ...
        pages = []
        plist = ["Hamid", "Cristiano Ronaldo", "zaim", "zaim", "zaim", "zaim", "hamid", "hamid","hamid","hamid","hamid","hamid","hamid","hamid","hamid"]
        ...
        for patient in pages[self.page]:
            ...
            patientcard = CTkFrame(
                self.patientsframe, width=350, height=100, corner_radius=20, fg_color=Colors.Coral
            )
        ...
        CTkLabel(self.patientsframe, text=self.page+1, font=font(30), text_color=Colors.Silver).place(x=190, y=523)


        self.previousButton = CTkButton(
            self.patientsframe,
            text="<",
            font=font(25),
            height=40,
